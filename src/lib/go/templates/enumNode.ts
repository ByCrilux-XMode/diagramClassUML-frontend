...
import { formatLoc, parseLoc } from "../converters";
import {
  backgroundShape,
  makePort,
  resizeAdornment,
...
export function createEnumNodeTemplate(): go.Node {
      ...
      new go.Panel("Auto")
        ...
        .add(
          new go.TextBlock({
            ...
            .bind("text", "name")
            .bind("font", "isAbstract", fontForClassNameFallback),
        )
        .add(separatorLine("literals"))
        .add(
          new go.Panel("Vertical", {
            stretch: go.GraphObject.Horizontal,
            itemTemplate: new go.Panel("Vertical", {
              ...
              new go.TextBlock({
                ...
                overflow: go.TextBlock.OverflowEllipsis,
                stretch: go.GraphObject.Horizontal,
              }).bind("text", "", (literal: string) => literal),
            ),
          }).bind("itemArray", "literals"),
          ),
        )
    )
...
function fontForClassNameFallback(): string {
  return fontSans("14px", "600");
}